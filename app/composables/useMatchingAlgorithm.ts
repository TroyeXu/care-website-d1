...
export const useMatchingAlgorithm = () => {
  const calculateLocationScore = (caregiver: Caregiver, criteria: MatchingCriteria): number => {
    ...
    } else if (caregiver.location.includes('台北') && criteria.location.includes('台北')) {
      return 20 // Medium score for same city
    } else if (caregiver.location.includes('新北') && criteria.location.includes('台北')) {
  ...
  const calculateSkillScore = (caregiver: Caregiver, criteria: MatchingCriteria): number => {
    if (!criteria.requiredSkills || criteria.requiredSkills.length === 0) return 0
  ...
  const calculateLicenseScore = (caregiver: Caregiver, criteria: MatchingCriteria): number => {
    if (!criteria.requiredLicenses || criteria.requiredLicenses.length === 0) return 0
  ...
  const calculateExperienceScore = (caregiver: Caregiver, criteria: MatchingCriteria): number => {
  ...
  const calculateRatingScore = (caregiver: Caregiver): number => {
    // Rating contributes up to 25 points (5.0 rating = 25 points)
    return Math.round(caregiver.rating * 5)
  }

  const calculateBudgetScore = (caregiver: Caregiver, criteria: MatchingCriteria): number => {
    if (!criteria.maxBudget) return 0
    
    const price = criteria.serviceType === 'shift' ? caregiver.shift_rate : caregiver.hourly_rate
  ...
  const calculateAvailabilityScore = (caregiver: Caregiver, criteria: MatchingCriteria): number => {
  ...
  const calculateSpecialNeedsScore = (caregiver: Caregiver, criteria: MatchingCriteria): number => {
    if (!criteria.patientConditions || criteria.patientConditions.length === 0) return 0
    ...
    for (const condition of criteria.patientConditions) {
      ...
      if (caregiverSkills.includes(conditionLower) || caregiverExperience.includes(conditionLower)) {
        score += 25 // High score for specific condition experience
      } else if (conditionLower.includes('失智') && caregiverSkills.includes('失智')) {
        score += 30 // Extra high for dementia care
      } else if (conditionLower.includes('復健') && caregiverSkills.includes('復健')) {
  ...
  const generateMatchReasons = (caregiver: Caregiver, criteria: MatchingCriteria, scores: Record<string, number>): string[] => {
    const reasons: string[] = []
    
    if (scores.rating >= 20) {
      reasons.push(`⭐ 高評分看護師 (${caregiver.rating}/5.0)`)
    }
  ...
  const generateWarnings = (caregiver: Caregiver, criteria: MatchingCriteria, scores: Record<string, number>): string[] => {
    ...
    if (scores.skills === 0 && criteria.requiredSkills && criteria.requiredSkills.length > 0) {
  ...
  const calculateMatchScore = (caregiver: Caregiver, criteria: MatchingCriteria): MatchResult => {
    const scores = {
    ...
    const totalScore = Math.round(Object.values(scores).reduce((sum, score) => sum + score, 0) * urgencyMultiplier)
    ...
    const priceEstimate = criteria.serviceType === 'shift' ? caregiver.shift_rate : caregiver.hourly_rate
  ...
  const findMatches = (caregivers: Caregiver[], criteria: MatchingCriteria, limit: number = 3): MatchResult[] => {
