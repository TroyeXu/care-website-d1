...
export const useBookingFilters = () => {
  ...
  const calculateMatchScore = (caregiver: Caregiver, preferences: MatchPreferences): number => {
    ...
    // 地區匹配 (25%)
    if (preferences.location && caregiver.location?.includes(preferences.location)) {
      score += 0.25
    }
    ...
    // 價格合理性 (20%)
    if (preferences.maxHourlyRate && caregiver.hourly_rate <= preferences.maxHourlyRate) {
      score += 0.2
    }
  ...
  const filterCaregivers = (caregivers: Caregiver[], filters: FilterConfig) => {
    return caregivers.filter(caregiver => {
      ...
      if (filters.location && filters.location.length > 0) {
        if (!filters.location.some(loc => caregiver.location?.includes(loc))) {
          return false
        }
      }
      ...
      // 評分篩選
      if (filters.rating) {
        if (caregiver.rating < filters.rating.min || caregiver.rating > filters.rating.max) {
          return false
        }
      }
      ...
      if (filters.hourlyRate) {
        if (caregiver.hourly_rate < filters.hourlyRate.min || 
