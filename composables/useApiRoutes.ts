export const useApiRoutes = () => {
  ...
  const routes = {
    ...
    bookings: {
      ...
      caregiverBookings: (caregiverId: number) => `${baseUrl}/bookings/caregiver/${caregiverId}`,
    ...
    reviews: {
      ...
      byCaregiver: (caregiverId: number) => `${baseUrl}/reviews/caregiver/${caregiverId}`,
  ...
  // API 呼叫配置
  const createApiConfig = (method: string, data?: any, options?: any) => {
    const config: any = {
      method,
      headers: {
